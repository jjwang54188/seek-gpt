...
export async function onRequest({request,env}:{request:Request;env:{VISITOR_DB?:any}}){
  ...
  try{
    ...
    if(action==='status')return reply({loggedIn:!!session?.state.cookie,pending:!!session?.state.qrKey,nickname:session?.state.nickname||'',expires:session?.row.expires||null});
    ...
    if(action==='start'){
      ...
      const identity='deviceId='+random();
      const response=await netease('/api/login/qrcode/unikey',{type:3},identity);const data=await response.json() as any;
      if(Number(data.code)!==200||typeof data.unikey!=='string'||!/^[\w-]{1,200}$/.test(data.unikey))return reply({error:'网易云暂时未提供登录二维码。'},502);
      ...
      const nmtid=(response.headers.get('set-cookie')||'').match(/(?:^|[,;]\s*)NMTID=([^;,\r\n]+)/)?.[1];
      const payload=await seal(secret,{qrKey:data.unikey,cookie:'',identity:identity+(nmtid?'; NMTID='+nmtid:'')});
    ...
    if(action==='poll'){
      ...
      const response=await netease('/api/login/qrcode/client/login',{key:session.state.qrKey,type:3},session.state.identity||'');const data=await response.json() as any;
      ...
      if(data.code===803){
        const headers=typeof response.headers.getSetCookie==='function'?response.headers.getSetCookie().join(', '):response.headers.get('set-cookie')||'';
        ...
        const cookie=[session.state.identity||'',...pairs].filter(Boolean).join('; ');let nickname='网易云用户';
