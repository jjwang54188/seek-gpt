...
export async function onRequestGet({request}: {request: Request}) {
  ...
  } else if (action === 'lyrics' || action === 'stream') {
  ...
  try {
    ...
    if (action === 'stream') {
      ...
      if (!song?.url) return Response.json({error:'网易云未提供此歌曲的可播音源，可能受版权、会员或地区限制。'},{status:403});
      const media=new URL(song.url);
      if (!media.hostname.endsWith('.music.126.net') || !['http:','https:'].includes(media.protocol)) return Response.json({error:'音源地址不受支持。'},{status:502});
      media.protocol='https:';
      return Response.json({url:media.href,trial:!!song.freeTrialInfo},{headers:{'Cache-Control':'no-store'}});
    }
  ...
  } catch (error) { return Response.json({error:'网易云暂时未返回可用数据，请稍后重试。',diagnostic:error instanceof Error?error.message.slice(0,200):'unknown'},{status:502,headers:{'Cache-Control':'no-store'}}); }
}
